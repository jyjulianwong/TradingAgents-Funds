...
def _build_prompt(isin: str, fact_sheet: str) -> list[dict[str, str]]:
    system = (
        ...
        "Selection rules:\n"
        "- If the fact sheet gives an explicit ticker/symbol per holding, pick "
        "verbatim from that column — do not alter or invent a ticker that "
        "isn't grounded in the fact sheet.\n"
        "- If the fact sheet lists holdings by company name only (no ticker "
        "column — some providers show name + SEDOL/ISIN instead of a "
        "ticker), identify the ticker yourself from your own knowledge of "
        "the company. Only do this when you are confident which exchange "
        "listing is meant (e.g. a well-known large-cap with one obvious "
        "primary listing); if a name is ambiguous, obscure, or multi-listed "
        "and you are not sure, leave it out rather than guessing.\n"
        ...
        "bonds/cash, or every name is too ambiguous to confidently ticker), "
