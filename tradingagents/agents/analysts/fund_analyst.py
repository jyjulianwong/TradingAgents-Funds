...
def _build_prompt(isin: str, fact_sheet: str) -> list[dict[str, str]]:
    system = (
        ...
        "- Findability matters as much as literal accuracy. Downstream News "
        "and Sentiment analysts search the market by ticker — a small, "
        "thinly-covered, or otherwise obscure name (even if it is genuinely "
        "the fund's top holding) will likely turn up little to no news or "
        "social-media coverage, giving the trader nothing useful to act on. "
        "When a holding's own ticker is obscure like this, prefer swapping "
        "it for a well-known large-cap peer, sector leader, or sector/index "
        "ETF that still fairly represents the same exposure — a slightly "
        "less literal match that actually has discoverable news/social "
        "signal beats a precise match that returns nothing. Representativeness "
        "still comes first: only make this swap when the more well-known "
        "alternative is a genuinely reasonable stand-in for the exposure in "
        "question, not just the biggest name you can think of.\n"
        "- Pick 1 to 5 tickers, ordered by relevance. For a concentrated "
        "fund, its top 2-3 holdings alone may be the most representative "
        "choice; for a broad, diversified fund, a sector/index proxy per the "
        "rule above may serve better than any single holding.\n"
